...
from recomp_overrides import (emitted_fingerprint, overrides_for_module,
...
def load(path):
    ...
    OVERRIDES.clear()
    # The PROJECT being emitted, not this translator's own repo. The
    # translator is shared -- it lives in `recomp-x86` and serves every x86
    # port -- so resolving from __file__ would scan the translator for
    # x86_register_override calls, find none, and emit a module with every
    # native override silently absent. That failure builds, links and runs.
    root = os.getcwd()
...
def cmd_emit(argv):
    ...
    lines = [PROLOGUE % (d["program"], translator_fingerprint(d["program"]),
                         base_sym, base_sym)]
...
def translator_fingerprint(program):
    """A content hash of every input that changes emitted call routing.
    ...
    what it emits just as much as a commit does. The authoritative
    `src/native` override-registration set is included too: adding an override
    changes direct callers from a C call to DISPATCH even when this Python file
    itself did not change.
    """
    return emitted_fingerprint(os.path.abspath(__file__), os.getcwd(), program)
...
def cmd_runtime(argv):
    ...
    L = ['/* generated by tools/recomp.py runtime -- do not edit */',
         STAMP % translator_fingerprint(d["program"]),
         '#include <windows.h>',
